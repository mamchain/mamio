...
bool CWallet::GetBalance(const CDestination& dest, const uint256& hashFork, int nForkHeight, CWalletBalance& balance)
{
    ...
    if (dest.IsTemplate() && dest.GetTemplateId().GetType() == TEMPLATE_MINTREDEEM)
    {
        int64 nTxPoolAmount = pTxPool->GetDestAmountLock(dest);
        if (nTxPoolAmount > 0)
        {
        ...
        else
        {
            ...
            int64 nRedeemLockAmount = pBlockChain->GetRedeemLockAmount(dest, nLastBlockBalance);
            if (nRedeemLockAmount < 0 || nRedeemLockAmount > balance.nAvailable)
            {
                nRedeemLockAmount = balance.nAvailable;
            }
            balance.nAvailable -= nRedeemLockAmount;
            balance.nLocked += nRedeemLockAmount;
        }
    }
                              ...
                              std::set<crypto::CPubKey>& setSignedKey, bool& fCompleted)
{
    if (destIn.IsPubKey())
    {
        ...
        if (!fCompleted)
        {
            StdError("CWallet", "Sign destination: PubKey SignPubKey fail, txid: %s, destIn: %s", tx.GetHash().GetHex().c_str(), CAddress(destIn).ToString().c_str());
        }
        return fCompleted;
    }
    ...
    if (destIn.IsTemplate())
    {
        ...
        {
            StdError("CWallet", "Sign destination: GetSignDestination fail, txid: %s", tx.GetHash().GetHex().c_str());
            return false;
        }
