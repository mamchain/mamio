...
static CBlockData BlockToJSON(const uint256& hashBlock, const CBlock& block, const uint256& hashFork, int nHeight)
{
    ...
    data.strProof = xengine::ToHexString(block.vchProof);
    char str_bits[10] = { 0 };
    sprintf(str_bits, "%x", block.nBits);
    data.strBits = str_bits;
...
void CRPCMod::JsonReply(uint64 nNonce, const std::string& result)
{
    ...
    eventHttpRsp.data.mapHeader["server"] = "minemon-rpc";
...
{
    pService->Stop();
    return MakeCStopResultPtr("minemon server stopping");
}

CRPCResultPtr CRPCMod::RPCVersion(CRPCParamPtr param)
{
    string strVersion = string("minemon server version is v") + VERSION_STR + string(", git commit id is ") + GetGitVersion();
...
CRPCResultPtr CRPCMod::RPCSignMessage(CRPCParamPtr param)
{
    ...
    {
        const string strMessageMagic = "Minemon Signed Message:\n";
        CBufStream ss;
        ss << strMessageMagic;
        ss << strMessage;
...
CRPCResultPtr CRPCMod::RPCGetWork(CRPCParamPtr param)
{
    ...
    if (spParam->nPledgefee > 1000)
    {
        throw CRPCException(RPC_INVALID_PARAMETER, "Invalid pledgefee");
    }
    uint32 nPledgeFee = spParam->nPledgefee;
    CTemplateMintPtr ptr = CTemplateMint::CreateTemplatePtr(new CTemplateProof(static_cast<CDestination&>(addrSpent), nPledgeFee));
    if (ptr == nullptr)
    {
    ...
    char str_bits[10] = { 0 };
    sprintf(str_bits, "%x", nBits);
    spResult->work.strBits = str_bits;
    spResult->work.strAux_Data = crypto::CryptoSHA256(vchWorkData.data(), vchWorkData.size()).GetHex();
    spResult->work.strMam_Data = xengine::ToHexString(vchWorkData);
...
CRPCResultPtr CRPCMod::RPCSubmitWork(CRPCParamPtr param)
{
    ...
    auto btc80 = xengine::ParseHexString(spParam->strBtc80);
    CProofOfHashWorkCompact proof;
    proof.SetBtcPow(btc80);
    proof.auxOffset = spParam->nCoinbaseindex;
    proof.coinbase = xengine::ParseHexString(spParam->strCoinbase);
    for (auto aux : spParam->vecMerkle_Branch_Aux)
    {
        uint256 obj;
        obj.SetHex(aux);
        std::reverse(obj.begin(), obj.end());
        proof.auxMerkleBranch.push_back(obj);
    }
    for (auto txid : spParam->vecMerkle_Branch)
    {
        uint256 obj;
        obj.SetHex(txid);
        std::reverse(obj.begin(), obj.end());
        proof.trMerkleBranch.push_back(obj);
    }
    std::vector<uint8> vchWorkData;
    proof.Save(vchWorkData);
    ...
    std::vector<uint8_t> mem_data = xengine::ParseHexString(spParam->strMam_Data);
    xengine::CBufStream ss1;
    ss1.Write((const char*)mem_data.data(), mem_data.size());
    ss1 >> nVersion >> nType >> hashBlock >> hashMerkle >> nBits;
    xengine::CBufStream ss2;
    ss2 << nVersion << nType << proof.btcTime << hashBlock << hashMerkle << nBits << vchWorkData;
    std::vector<uint8> data;
    data.assign(ss2.GetData(), ss2.GetData() + ss2.GetSize());

    Errno err = pService->SubmitWork(data, ptr, hashBlock);
    if (err != OK)
    {
