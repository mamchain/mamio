...
{
    if (!config.Load(argc, argv, GetDefaultDataDir(), "minemon.conf") || !config.PostLoad())
    {
    ...
    {
        cout << "minemon version is v" << VERSION_STR << ", git commit id is " << GetGitVersion() << endl;
        return false;
    }
    ...
    if (config.GetConfig()->fDaemon && (config.GetModeType() == EModeType::SERVER || config.GetModeType() == EModeType::MINER))
    {
        ...
        cout << "minemon server starting\n";
    }

    // log
    if ((config.GetModeType() == EModeType::SERVER || config.GetModeType() == EModeType::MINER)
        && log.SetLogFilePath((pathData / "minemon.log").string())
    ...
    {
        cerr << "Failed to open log file : " << (pathData / "minemon.log") << "\n";
        return false;
    }
        ...
        && (config.GetConfig()->fCheckRepair || config.GetConfig()->fOnlyCheck))
    {
        ...
        if (!check.CheckRepairData())
        {
            ...
            {
                StdError("minemon", "Check data fail.");
            }
            else
            {
                StdError("minemon", "Check and repair data fail.");
            }
            return false;
        }
        if (config.GetConfig()->fOnlyCheck)
        {
            StdLog("minemon", "Check data complete.");
            return false;
        }
        StdLog("minemon", "Check and repair data complete.");
    }

#if !defined(WIN32) && !defined(__APPLE__)
    StdLog("minemon", "malloc_trim: %d.", malloc_trim(0));
#endif
    ...
    StdLog("MinemonStartup", "Initialize: minemon version is v%s, git commit id: %s", VERSION_STR.c_str(), GetGitVersion());
        ...
        case EModuleType::LOCK:
        {
            if (!TryLockFile((config.GetConfig()->pathData / ".lock").string()))
            {
                cerr << "Cannot obtain a lock on data directory " << config.GetConfig()->pathData << "\n"
                     << "Minemon is probably already running.\n";
                return false;
            }
            break;
        }
...
{
    // Windows: C:\Documents and Settings\username\Local Settings\Application Data\Minemon
    // Mac: ~/Library/Application Support/Minemon
    // Unix: ~/.minemon
    ...
    {
        return path(pszPath) / "MINEMON";
    }
    return path("C:\\MINEMON");
#else
    path pathRet;
...
bool CBbEntry::RunInBackground(const path& pathData)
{
    ...
    if (pid > 0)
    {
        FILE* file = fopen((pathData / "minemon.pid").string().c_str(), "w");
        if (file)
        {
            fprintf(file, "%d\n", pid);
            fclose(file);
...
{
#ifndef WIN32
    boost::filesystem::remove(pathData / "minemon.pid");
#endif
}
