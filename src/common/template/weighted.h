...
{
public:
    CTemplateWeighted(const uint8 nRequiredIn = 0, const std::map<minemon::crypto::CPubKey, uint8>& mapPubKeyWeightIn = std::map<minemon::crypto::CPubKey, uint8>());
...
protected:
    uint8 nRequired;
    std::map<minemon::crypto::CPubKey, uint8> mapPubKeyWeight;
};
