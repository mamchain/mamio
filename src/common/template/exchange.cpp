    ...
    fork_s(fork_s_)
{
    ...
    nId = CTemplateId(nType, minemon::crypto::CryptoHash(vchData.data(), vchData.size()));
}
...
void CTemplateExchange::GetTemplateData(minemon::rpc::CTemplateResponse& obj, CDestination&& destInstance) const
{
    ...
    obj.exchange.strSpend_M = minemon::CAddress(destSpend_m).ToString();
    obj.exchange.strSpend_S = minemon::CAddress(destSpend_s).ToString();
}
...
bool CTemplateExchange::SetTemplateData(const minemon::rpc::CTemplateRequest& obj, CDestination&& destInstance)
{
    ...
    minemon::CAddress address_m(addr_m);
    minemon::CAddress address_s(addr_s);
