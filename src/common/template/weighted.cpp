...
CTemplateWeighted::CTemplateWeighted(const uint8 nRequiredIn, const std::map<minemon::crypto::CPubKey, uint8>& mapPubKeyWeightIn)
...
void CTemplateWeighted::GetTemplateData(minemon::rpc::CTemplateResponse& obj, CDestination&& destInstance) const
{
    obj.weighted.nRequired = nRequired;
    for (const auto& keyweight : mapPubKeyWeight)
    {
        minemon::rpc::CTemplatePubKeyWeight pubkey;
...
bool CTemplateWeighted::SetTemplateData(const minemon::rpc::CTemplateRequest& obj, CDestination&& destInstance)
{
