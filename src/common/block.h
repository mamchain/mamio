...
public:
    ...
    uint256 GetHash() const
    {
        ...
        ss << nVersion << nType << nTimeStamp << hashPrev << hashMerkle << nBits << vchProof;
    ...
    {
        return (sizeof(nVersion) + sizeof(nType) + sizeof(nTimeStamp) + sizeof(hashPrev) + sizeof(hashMerkle) + sizeof(nBits) + xengine::GetSerializeSize(vchProof));
    }
    ...
    uint256 BuildMerkleTree(std::vector<uint256>& vMerkleTree) const
    {
        ...
        for (int nSize = vtx.size() + 1; nSize > 1; nSize = (nSize + 1) / 2)
        {
            for (int i = 0; i < nSize; i += 2)
            {
                ...
                vMerkleTree.push_back(minemon::crypto::CryptoHash(vMerkleTree[j + i], vMerkleTree[j + i2]));
            }
            j += nSize;
        }
