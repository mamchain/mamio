...
if [ "$runonce" -eq 1 ]; then
    ...
    # import receiver
    ret=`minemon importprivkey 20e6078147f2d4ee757b71584f64d088519fd4a425780bdb630d45bc89c8fe1e 123`
	echo $(date -Iseconds)     $ret | tee -a $logfile
    # import genesis
    ret=`minemon importprivkey 9df809804369829983150491d1086b99f6493356f91ccc080e661a76a976a4ee 123`
	echo $(date -Iseconds)     $ret | tee -a $logfile

    # charge to sender from genesis
    ret=`minemon unlockkey da915f7d9e1b1f6ed99fd816ff977a7d1f17cc95ba0209eef770fb9d00638b49 123`
	echo $(date -Iseconds)     $ret | tee -a $logfile
    ret=`minemon sendfrom 1965p604xzdrffvg90ax9bk0q3xyqn5zz2vc9zpbe3wdswzazj7d144mm 1erazk3ww4nap6bm46mfct98axe2phsqcba00987ezqqdpswfv6yexz7a 10000000.00 11.26`
	echo $(date -Iseconds)     $ret | tee -a $logfile
fi
