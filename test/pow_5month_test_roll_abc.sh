...
# check if minemon has run right now, otherwise start it up.
instance=$(ps aux | grep "$app" | grep -v "console" | grep -v "grep" | grep -v ".log" | wc -l)
if [ "$instance" -le 1 ]; then
    $app -daemon -debug
	echo $(date -Iseconds)     minemon has started. | tee -a $logfile
    echo 'Waiting for minemon daemon running up...' | tee -a $logfile
    sleep 10
fi

#sha256address=1erazk3ww4nap6bm46mfct98axe2phsqcba00987ezqqdpswfv6yexz7a
#sha256key=20e6078147f2d4ee757b71584f64d088519fd4a425780bdb630d45bc89c8fe1e
runonce=1
if [ "$runonce" -eq 1 ]; then
    # import address A
    ret=`minemon importprivkey fa7390d1bf9fff170827d5eca4bde011f56064ac6a63e3ab41872a0ab5afcf6b 123`
	echo $(date -Iseconds)     $ret | tee -a $logfile
    # import address B
    ret=`minemon importprivkey 7ff746e5ea507937ad4660b72b664bdc4a71909691638e78db596513f713c6fb 123`
	echo $(date -Iseconds)     $ret | tee -a $logfile
    # import address C
    ret=`minemon importprivkey 992766ac5c6d961623188d947adbe594d6b89abe1cd32b301d4b2139d5c40dc5 123`
	echo $(date -Iseconds)     $ret | tee -a $logfile

    # import mint template address
    ret=`minemon addnewtemplate mint '{"mint": "9513efbd24eddf2cb70f7bca1b23cd692b4c6af5bf6dbe92bda5b3cccf61498d", "spent": "1erazk3ww4nap6bm46mfct98axe2phsqcba00987ezqqdpswfv6yexz7a"}'`
	echo $(date -Iseconds)     $ret | tee -a $logfile
    # import mint spend address
    ret=`minemon importprivkey f43fab79c29ebb94cc680c97d5505c0c95842f0fa345d08e2839a85f60e6e38a 123`
	echo $(date -Iseconds)     $ret | tee -a $logfile
fi
...
function charge
{
    ...
    # unlock spend key
    ret=`minemon unlockkey bcd98f67dbeefdeea004805aece66885eb0a25cd1e35842e6355259c8ff91576 123`
	echo $(date -Iseconds)     $ret | tee -a $logfile
	# charge A from mint
    ret=`minemon sendfrom 20g0bdg20sx9ajssd39ghzbrjtphqcb0q1ng40skeahh2pa6xghbnxgw1 123bd9kvh4gze86yr8d1gqscebx0zs4z8wc270py227h8cres28qf4xeg 1100.00 3.0`
	echo $(date -Iseconds)     $ret | tee -a $logfile
}
