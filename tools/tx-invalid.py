...
def get_block_hash(height):
    json_str = str(subprocess.check_output(
        ['minemon', 'getblockhash', str(height)]))
...
def get_block(block_hash):
    json_str = subprocess.check_output(['minemon', 'getblock', block_hash])
    return json.loads(json_str)

def get_transaction(tx_hash):
    json_str = subprocess.check_output(['minemon', 'gettransaction', tx_hash])
    json_obj = json.loads(json_str)
    json_obj['transaction']

def get_primary_fork_height():
    return int(subprocess.check_output(['minemon', 'getforkheight']))
